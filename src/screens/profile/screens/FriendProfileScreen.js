...
// IMPORT COMPONENT
import { FriendActionSheet } from '../components/ui/FriendComponents';
...
const FriendProfileScreen = () => {
  ...
  useEffect(() => {
    const loadProfile = async () => {
      ...
      try {
        ...
        else { showToast(response.message || "Could not load user profile.", 'error'); }
      } catch (error) {
  ...
  const handleBlock = () => {
    ...
    showAlert({
      ...
      message: "They won't be able to send you messages or requests.",
      type: 'error',
      btnText: 'Block',
      onClose: async () => {
        try { await blockUser(user.name); showToast(`${user.name} has been blocked`, 'info'); navigation.goBack(); } 
  ...
  const handleUnblock = () => {
    ...
    showAlert({
      ...
      btnText: 'Unblock',
      onClose: async () => {
        try { await unblockUser(user.id); showToast(`You unblocked ${user.name}`, 'success'); } 
        catch (error) { showToast("Failed to unblock user", 'error'); }
      },
      secondaryBtnText: 'Cancel'
    });
  };
  ...
  const handleUnfriend = () => {
    ...
    showAlert({
        ...
        btnText: 'Unfriend',
        onClose: async () => {
            try { await removeFriend(user.id); showToast(`${user.name} removed from friends`, 'info'); } 
  ...
  const handleAddFriend = () => {
    ...
    addFriend(user.id).catch(() => { showToast('Failed to send request', 'error'); });
  };
  ...
  const handleReport = () => {
     ...
     showAlert({
        ...
        message: 'Your report will be reviewed by our moderation team.',
        type: 'info',
        btnText: 'Submit Report',
        onClose: () => { showToast('Report submitted', 'success'); },
        secondaryBtnText: 'Cancel'
    });
  };
  ...
  if (!user) return <View style={[styles.container, styles.center]}><Ionicons name="alert-circle-outline" size={48} color={Colors.textSecondary} /><Text style={{color: Colors.textSecondary, marginTop: 10}}>User not found</Text><TouchableOpacity onPress={() => navigation.goBack()} style={styles.goBackButton}><Text style={{color: Colors.text}}>Go Back</Text></TouchableOpacity></View>;
  ...
  return (
    <View style={styles.container}>
      ...
      <TouchableOpacity style={[styles.backBtn, { top: insets.top + 10 }]} onPress={() => navigation.goBack()} activeOpacity={0.8}>
        <Ionicons name="arrow-back" size={24} color="#FFF" />
      </TouchableOpacity>

      <Animated.ScrollView onScroll={Animated.event([{ nativeEvent: { contentOffset: { y: scrollY } } }], { useNativeDriver: true })} scrollEventThrottle={16} showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 50 }}>
        <View style={styles.heroContainer}>
          <Image source={{ uri: bannerUri || 'https://via.placeholder.com/800' }} style={styles.banner} resizeMode="cover" />
        ...
        <View style={styles.profileContent}>
          ...
          <View style={styles.actionRow}>
            {isSelf ? (
                ...
                <>
                    <TouchableOpacity style={[styles.primaryBtn, isBlocked && styles.disabledBtn]} onPress={() => !isBlocked && navigation.navigate('ChatDetail', { user })} activeOpacity={isBlocked ? 1 : 0.7}>
                        <Ionicons name={isBlocked ? "ban" : "chatbubble-ellipses"} size={20} color={isBlocked ? Colors.textSecondary : "#FFF"} />
                        <Text style={[styles.btnText, isBlocked && { color: Colors.textSecondary }]}>{isBlocked ? "Blocked" : "Message"}</Text>
                    </TouchableOpacity>
          ...
          {badges.length > 0 ? (
            ...
            <View style={styles.emptyState}><Text style={styles.emptyText}>No trophies yet.</Text></View>
          )}
        </View>
      </Animated.ScrollView>
