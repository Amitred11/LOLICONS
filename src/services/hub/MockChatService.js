...
export const ChatAPI = {
  ...
  votePoll: async (chatId, messageId, optionId) => {
    ...
    if (msg && msg.type === 'poll') {
        const userId = 'me';
        const userDetails = { id: 'me', name: 'You', avatar: null }; // Full voter object

        // Find if I voted for the clicked option already
        const optionToVote = msg.poll.options.find(opt => opt.id === optionId);
        const didIVoteForThisOption = optionToVote.voters.some(v => v.id === userId);

        // Remove my vote from ALL options first (enforces single choice)
        ...
        // If I wasn't already voting for this option, add my vote
        if (!didIVoteForThisOption) {
            optionToVote.voters.push(userDetails);
        }

        // Recalculate votes for all options and the total
        let totalVotes = 0;
        msg.poll.options.forEach(opt => {
            opt.votes = opt.voters.length;
            totalVotes += opt.votes;
        });
        msg.poll.totalVotes = totalVotes;
  ...
  reactToMessage: async (chatId, messageId, reaction) => {
    ...
    if (msg) {
        ...
        const userId = 'me';
        const myExistingReactionKey = Object.keys(msg.reactions).find(key => msg.reactions[key].includes(userId));

        // Remove my existing reaction if there is one
        if (myExistingReactionKey) {
            msg.reactions[myExistingReactionKey] = msg.reactions[myExistingReactionKey].filter(id => id !== userId);
            if (msg.reactions[myExistingReactionKey].length === 0) {
                delete msg.reactions[myExistingReactionKey];
            }
        }
        
        // Add the new reaction, unless it's the one we just removed (toggle off)
        if (myExistingReactionKey !== reaction) {
  ...
  deleteMessage: async (chatId, messageId, deleteType) => {
    ...
    if (deleteType === 'for_me') {
        ...
        const msgIndex = MOCK_MESSAGES[chatId].findIndex(m => m.id === messageId);
        if (msgIndex > -1) {
            const originalSender = MOCK_MESSAGES[chatId][msgIndex].sender;
            MOCK_MESSAGES[chatId][msgIndex] = {
                ...MOCK_MESSAGES[chatId][msgIndex],
                type: 'system', 
                text: 'This message was deleted',
                isDeleted: true,
                sender: originalSender, // Keep sender to differentiate "You deleted..."
                imageUri: null,
                fileName: null,
                poll: null,
                reactions: {}
            };
            return { success: true, action: 'updated', data: MOCK_MESSAGES[chatId][msgIndex] };
        }
    }
    return { success: false };
  },
  
  // ... other functions from original file are unchanged ...
  ...
  addParticipantsToGroupCall: async (chatId, newUserIds) => {
    ...
    const newMembers = newUserIds.map(id => {
        ...
        return { id: user.id, name: user.name, avatar: user.avatarUrl, isMuted: true, isCameraOn: false };
    });
  ...
  sendInCallMessage: async (text) => {
      await delay(200);
      const newMsg = { id: `incall_${Date.now()}`, text, sender: 'me', senderName: 'You', time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })};
      MOCK_IN_CALL_MESSAGES.push(newMsg);
      return { success: true, data: newMsg };
  },
  getInCallMessages: () => { return MOCK_IN_CALL_MESSAGES; },
  clearInCallMessages: () => { MOCK_IN_CALL_MESSAGES = []; },
  clearHistory: async (chatId) => { await delay(800); MOCK_MESSAGES[chatId] = []; return { success: true }; },
  toggleMute: async (chatId, isMuted) => { await delay(400); const chat = MOCK_CHATS.find(c => c.id === chatId); if(chat) chat.isMuted = isMuted; return { success: true }; },
  blockUser: async (userId) => { await delay(1000); MOCK_CHATS = MOCK_CHATS.filter(c => c.id !== userId); return { success: true }; },
  reportUser: async (userId, reason) => { await delay(800); return { success: true }; },
  leaveGroup: async (chatId) => { await delay(1200); MOCK_CHATS = MOCK_CHATS.filter(c => c.id !== chatId); return { success: true }; },
  updateChatSettings: async (chatId, settings) => { await delay(500); const chatIndex = MOCK_CHATS.findIndex(c => c.id === chatId); if (chatIndex === -1) return { success: false }; if (settings.disappearingMessages !== undefined) { MOCK_CHATS[chatIndex].disappearingMessages = { ...MOCK_CHATS[chatIndex].disappearingMessages, enabled: settings.disappearingMessages }; } return { success: true, data: MOCK_CHATS[chatIndex] }; },
  fetchFriends: async () => { await delay(600); const users = Object.values(MOCK_ALL_USERS || {}).filter(u => u.id !== 'usr_a1b2c3d4e5f6g7h8').map(u => ({ ...u, avatar: u.avatarUrl })); return { success: true, data: users }; },
  addMembersToGroup: async (chatId, newMemberIds) => { await delay(1000); const chat = MOCK_CHATS.find(c => c.id === chatId); if (chat) { const allUsers = Object.values(MOCK_ALL_USERS || {}); const newMembers = newMemberIds.map(id => { const user = allUsers.find(u => u.id === id); return user ? { ...user, avatar: user.avatarUrl } : null; }).filter(Boolean); let currentMembers = chat.members || []; const currentMemberIds = currentMembers.map(m => m.id || m); const uniqueNewMembers = newMembers.filter(m => !currentMemberIds.includes(m.id)); chat.members = [...currentMembers, ...uniqueNewMembers]; return { success: true, data: chat.members }; } return { success: false, message: 'Chat not found' }; },
  updateGroupProfile: async (chatId, updates) => { await delay(800); const chat = MOCK_CHATS.find(c => c.id === chatId); if (chat) { if (updates.name) chat.name = updates.name; if (updates.avatar) chat.avatar = updates.avatar; return { success: true, data: chat }; } return { success: false, message: 'Chat not found' }; },
  removeMemberFromGroup: async (chatId, userId) => { await delay(1000); const chat = MOCK_CHATS.find(c => c.id === chatId); if (chat) { chat.members = chat.members.filter(m => (m.id || m) !== userId); return { success: true, data: chat.members }; } return { success: false }; },
  setMemberNickname: async (chatId, userId, nickname) => { await delay(500); const chat = MOCK_CHATS.find(c => c.id === chatId); if(chat) { const memberIndex = chat.members.findIndex(m => (m.id || m) === userId); if(memberIndex > -1) { if(typeof chat.members[memberIndex] === 'string') { chat.members[memberIndex] = { id: chat.members[memberIndex], name: chat.members[memberIndex] }; } chat.members[memberIndex].nickname = nickname; } return { success: true }; } return { success: false }; },
};